  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 p-6">
      {product.map((item) => (
        <div
          key={item.id}
          className="border p-4 rounded-md shadow-md text-center bg-white"
        >
          <img src={item.img} alt={item.name} className="h-40 mx-auto object-contain" />
          <h2 className="text-lg font-semibold mt-2">{item.name}</h2>
          <p className="text-red-600 font-bold">
  ${item.aksiya ? (
    <>
      <span className="line-through text-gray-500">${item.price}</span>{' '}
      ${Math.floor(item.price * (1 - parseInt(item.aksiya) / 100))}
    </>
  ) : (
    <>${item.price}</>
  )}
          ...
          <button className="bg-black text-white px-4 py-1 mt-2 rounded">
            Add to Cart
          </button>
        </div>
      ))}
